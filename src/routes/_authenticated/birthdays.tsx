...
import {
  ageFromDob, daysUntil, initialsOf, upcomingBirthdayDate, type Member,
} from "@/lib/pfms";
...
function BirthdaysPage() {
  const members = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("members")
        .select("id, full_name, date_of_birth, phone, whatsapp, photo_url, branch_id, department_id")
        .not("date_of_birth", "is", null);
      if (error) throw error;
      return (data ?? []) as Pick<Member, "id" | "full_name" | "date_of_birth" | "phone" | "whatsapp" | "photo_url" | "branch_id" | "department_id">[];
    },
  });
  ...
  return (
    <div className="space-y-4">
      <PageHeader title="Birthdays" description={`${total} member${total === 1 ? "" : "s"} with birthday on file.`} />

      {total === 0 ? (
        <EmptyState icon={Cake} title="No birthdays yet" description="Members with a date of birth will appear here." />
      ) : (
        <>
...
function BirthdaySection({
  ...
  title: string;
  items: { m: { id: string; full_name: string; date_of_birth: string | null; phone: string | null; whatsapp: string | null; photo_url: string | null }; next: Date; days: number }[];
  highlight?: boolean;
}) {
  ...
  return (
    <section>
      ...
      <Card className={highlight ? "divide-y divide-border overflow-hidden ring-1 ring-primary/30" : "divide-y divide-border overflow-hidden"}>
            ...
            <div key={m.id} className="px-4 py-3 flex items-center gap-3">
              ...
              {wa && (
                <Button asChild size="sm" variant="outline">
                  <a href={`https://wa.me/${wa}?text=${encodeURIComponent(`Happy birthday, ${m.full_name.split(" ")[0]}! 🎉`)}`} target="_blank" rel="noopener noreferrer">
