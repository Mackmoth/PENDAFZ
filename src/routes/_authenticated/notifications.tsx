...
import { formatRelativeTime, labelize, type Notification } from "@/lib/pfms";
    ...
    <PageTransition>
      <div className="space-y-6">
        <PageHeader
          title="Notifications"
          description={
            list.data ? (unread ? `${unread} unread` : "You're all caught up") : undefined
          }
          action={
            unread > 0 ? (
          ...
          <Card className="divide-y divide-border overflow-hidden animate-fade-in">
            {list.data!.map((n) => (
              <button
                ...
                className={`w-full text-left px-4 py-3 hover:bg-accent transition-all ${!n.read_at ? "bg-primary/5 hover:bg-primary/10" : ""}`}
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      {!n.read_at && (
                        <span className="w-2 h-2 rounded-full bg-primary shrink-0 animate-pulse-soft" />
                      )}
