      ...
      <Card className="p-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-1">
            ...
            <Button variant="outline" size="sm" onClick={() => { const t = new Date(); setCursor(new Date(t.getFullYear(), t.getMonth(), 1)); }}>Today</Button>
            <Button variant="outline" size="icon" onClick={() => setCursor(new Date(cursor.getFullYear(), cursor.getMonth() + 1, 1))}>
        ...
        <div className="grid grid-cols-7 gap-px bg-border rounded-md overflow-hidden">
          {cells.map((d, i) => (
            ...
            >
              {d !== null && (
                <>
                  <div className={cn("font-medium", isToday(d) ? "text-primary" : "text-foreground")}>{d}</div>
                  <div className="mt-1 space-y-1">
                    {(eventsByDay[d] ?? []).slice(0, 2).map((e) => (
                      <div key={e.id} className="truncate text-[10px] bg-primary/10 text-primary rounded px-1 py-0.5">
                        {e.title}
                      </div>
                    ))}
                    {(birthdaysByDay[d] ?? []).slice(0, 2).map((b) => (
                      <div key={b.id} className="truncate text-[10px] bg-success/10 text-success rounded px-1 py-0.5 flex items-center gap-1">
                        <Cake className="w-2.5 h-2.5" />{b.full_name.split(" ")[0]}
                      </div>
                    ))}
            ...
            <ul className="space-y-2">
              {events.data!.map((e) => (
                <li key={e.id} className="flex items-center justify-between text-sm">
                  ...
                  <Badge variant="outline" className="text-[10px]">{fmtDate(e.event_date, { month: "short", day: "numeric" })}</Badge>
                </li>
              ))}
            </ul>
          )}
                ...
                .flatMap(([day, list]) =>
                  list.map((b) => (
                    <li key={b.id} className="flex items-center justify-between text-sm">
                      ...
                      <Badge variant="outline" className="text-[10px]">{monthLabel.split(" ")[0].slice(0, 3)} {day}</Badge>
                    </li>
                  )),
                )}
            </ul>
