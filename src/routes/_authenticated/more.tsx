  ...
  return (
    <PageTransition>
      <div className="space-y-6">
        ...
        <Card className="divide-y divide-border overflow-hidden animate-fade-in animate-fade-in-delay-1">
          {MODULES.filter((m) => !m.adminOnly || isAdmin).map((m) => {
            ...
            return (
              <Link
                key={m.label}
                to={m.to}
                className="w-full flex items-center gap-3 px-4 py-3.5 hover:bg-accent active:scale-[0.995] transition-all"
              >
